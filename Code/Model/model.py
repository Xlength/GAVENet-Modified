...
class UNetModule(nn.Module):
    def __init__(self, input_ch, output_ch, base_ch):
        ...
        # ===== ViT channel bottleneck: reduce 16*base_ch to a smaller channel (64), ViT works on the small channel =====
        C = 16 * base_ch        # Original channel, if base_ch=64 then C=1024
        C_reduced = 64          # Bottleneck channels: 64/96/128, 64 is the most memory-efficient

        # First reduce the dimension, then increase it back
        self.pre_vit_reduce = nn.Conv2d(C, C_reduced, kernel_size=1, bias=False)
        self.post_vit_expand = nn.Conv2d(C_reduced, C, kernel_size=1, bias=False)

        # ViT works with small channels
        self.vit = timm.create_model(
            'vit_base_patch16_224',
            pretrained=False,          # Keep False, the shape is most stable
            num_classes=C_reduced,     # The output dimension is C_reduced
            in_chans=C_reduced,        # Input channels are C_reduced
            global_pool='avg'
        )

        # Linear expansion is also done by C_reduced
        self.linear_expand = nn.Linear(
            C_reduced,
            C_reduced * (self.vit_img_size // 32) ** 2  # =C_reduced*7*7
        )

        # Make a 1x1 projection of the small channel features after ViT (still in the C_reduced channel)
        self.out_proj = nn.Conv2d(C_reduced, C_reduced, 1)
    ...
    def forward(self, x):
        ...
        # ===== ViT channel bottleneck forward: first reduce x to C_reduced, then go through ViT, and finally raise it back to C =====
        x_small = self.pre_vit_reduce(x)  # [B, C_reduced, H, W]
        B, C_r, H, W = x_small.shape

        # Input ViT (224x224), get [B, C_reduced]
        x_vit_in = F.interpolate(x_small, size=(self.vit_img_size, self.vit_img_size),
                                 mode='bilinear', align_corners=False)
        vit_out = self.vit(x_vit_in)  # [B, C_r]

        # Linearly expand back to the 7x7 of the small channel, and then interpolate back to the original spatial size
        x_small = self.linear_expand(vit_out).view(
            B, C_r, self.vit_img_size // 32, self.vit_img_size // 32
        )
        x_small = F.interpolate(x_small, size=(H, W), mode='bilinear', align_corners=False)
        x_small = self.out_proj(x_small)   #  [B, C_reduced, H, W]

        # Return to original channel C, subsequent decoders remain unchanged
        x = self.post_vit_expand(x_small)  # [B, C, H, W]

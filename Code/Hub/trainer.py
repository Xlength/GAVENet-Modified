...
class R2Vessels:
    ...
    def train_epoch(self, r2v_loader):
        ...
        # use tqdm 
        pbar = tqdm(r2v_loader, desc=f"[Train] Iter {self.iter}", ncols=100)
        ...
        for sample in pbar:
            ...
            # Real-time update of loss display
            pbar.set_postfix(loss=f"{loss.item():.4f}")

        avg_loss = total_loss / len(r2v_loader)
        print(f"\n|{self.iter}| [{self.model_name}, {self.criterion_name}] >> training epoch mean loss: {avg_loss:.6f}")
        return [avg_loss]
    ...
    def test(self, r2v_dataloader, prefix_to_save=None):
        with torch.no_grad():
            ...
            self.model.eval()
            self.model.cpu()
            pbar = tqdm(r2v_dataloader, desc=f"[Val] Iter {self.iter}", ncols=100)
            ...
            for sample in pbar:
                ...
                # I changed it to CPU verification here because my GPU memory is full.
                retino = data[0].to('cpu')
                vessels = data[1].to('cpu')
                mask = data[2].to('cpu')
            ...
            print(f"|{self.iter}| [{self.model_name}, {self.criterion_name}] >> validation mean loss: {avg_loss:.6f}")
            self.model.cuda()  #  After verification is complete, restore to GPU to avoid crash during training
            return [avg_loss]
    ...
    def training(
        ...
        print(f"🔧 Start training with model={self.model_name}, criterion={self.criterion_name}")
